...
Logique métier déplacée dans `_appliquer_regles_but` (privée), appelée
par `ButAgent.analyze()`. `analyze()` ne pilote jamais le Rule Engine
(Règle 9). `enrich_ticket()` reste un adaptateur de compatibilité. Point
d'attention : BUT a DEUX branches de sortie (matériel non détecté =
retour anticipé, matériel détecté = fin normale) -- `_appliquer_regles_but`
couvre les deux ; l'adaptateur applique le Rule Engine (si demandé) sur
le résultat final, quelle que soit la branche empruntée.
"""
...
def _appliquer_regles_but(ticket: Ticket, texte_mail: str = "") -> Ticket:
    """Logique métier BUT pure (extraite de l'ancien `enrich_ticket`, sans le bloc Rule Engine, 2 branches de sortie)."""
    notes: list[str] = []
...
class ButAgent(BaseAgent):
    """Agent BUT conforme au contrat BaseAgent. Cf. `_appliquer_regles_but` pour la logique métier."""

    def analyze(self, request: AgentRequest) -> AgentResult:
        try:
            ticket = _appliquer_regles_but(request.ticket, texte_mail=request.texte_mail)
...
def enrich_ticket(
    ...
    """⚠️ ADAPTATEUR DE COMPATIBILITÉ -- délègue à ButAgent.analyze(), reproduit ici l'ancien comportement de activer_rule_engine (les 2 branches sont couvertes puisqu'appliqué après analyze(), quel que soit le résultat)."""
