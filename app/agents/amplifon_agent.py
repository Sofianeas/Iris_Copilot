...
Logique métier déplacée dans `_appliquer_regles_amplifon` (privée),
appelée par `AmplifonAgent.analyze()`. `analyze()` ne pilote jamais le
...
def _appliquer_regles_amplifon(ticket: Ticket, texte_mail: str = "") -> Ticket:
    """Logique métier AMPLIFON pure (extraite de l'ancien `enrich_ticket`, sans le bloc Rule Engine)."""
    notes: list[str] = []
...
class AmplifonAgent(BaseAgent):
    """Agent AMPLIFON conforme au contrat BaseAgent. Cf. `_appliquer_regles_amplifon` pour la logique métier."""

    def analyze(self, request: AgentRequest) -> AgentResult:
        try:
            ticket = _appliquer_regles_amplifon(request.ticket, texte_mail=request.texte_mail)
            return AgentResult(ticket=ticket, succes=True)
        except Exception as exc:
            return AgentResult(ticket=request.ticket, succes=False, erreur=str(exc))


_AGENT = AmplifonAgent()
...
def enrich_ticket(
    ...
    """⚠️ ADAPTATEUR DE COMPATIBILITÉ -- délègue à AmplifonAgent.analyze(), reproduit ici l'ancien comportement de activer_rule_engine."""
