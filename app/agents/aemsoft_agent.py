...
# app/agents/aemsoft_agent.py
"""
Agent AEMSOFT (AEM SOFTS).

[... docstring métier inchangé, cf. versions précédentes ...]

--- Harmonisation Framework des Agents (P3-424) ---
La logique métier a été déplacée dans `_appliquer_regles_aemsoft` (privée),
appelée par `AemsoftAgent.analyze()` (nouveau contrat officiel). Le
paramètre `activer_rule_engine` n'existe plus dans `analyze()` -- l'agent
ne pilote jamais le Rule Engine (Règle 9 du Framework). L'ancienne
fonction `enrich_ticket()` reste un ADAPTATEUR DE COMPATIBILITÉ : elle
appelle `analyze()` puis reproduit elle-même l'ancien comportement de
`activer_rule_engine=True` (application du Rule Engine), pour ne casser
...
TYPE_DATE_IMPOSEE = "Intervention date imposée"
NIVEAU_SERVICE_DATE_IMPOSEE = "Date imposée"
NIVEAU_SERVICE_GTI_1J = "GTI 1J (5/7)"
SOUS_TYPE_AEM = "Intervention AVEC pièces expédiées par AEM"
SOUS_TYPE_IRIS = "Intervention AVEC pièces expédiées par IRIS"
SOUS_TYPE_SANS_PIECE = "Intervention SANS pièces"
CONTRAT_AEMSOFT = ""
...
def _appliquer_regles_aemsoft(ticket: Ticket, texte_mail: str = "") -> Ticket:
    """
    Logique métier AEMSOFT pure (extraite de l'ancien `enrich_ticket`,
    MOINS le bloc Rule Engine -- cf. Règle 9 du Framework : un agent ne
    pilote jamais le Rule Engine). Appelée par `AemsoftAgent.analyze()`.
    """
    notes: list[str] = []
...
class AemsoftAgent(BaseAgent):
    """Agent AEMSOFT conforme au contrat BaseAgent. Cf. `_appliquer_regles_aemsoft` pour la logique métier."""

    def analyze(self, request: AgentRequest) -> AgentResult:
        try:
            ticket = _appliquer_regles_aemsoft(request.ticket, texte_mail=request.texte_mail)
...
def enrich_ticket(
    ...
    """
    ⚠️ ADAPTATEUR DE COMPATIBILITÉ -- délègue à AemsoftAgent.analyze()
    puis reproduit ici (jamais dans analyze(), cf. Règle 9 du Framework)
    l'ancien comportement de `activer_rule_engine=True`.
    """
